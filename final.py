...
#step 2
    ...
    if GPIO.event_detected(Button_next):
        print("1. 중구, 2. 서구, 3. 동구, 4. 영도구, 5. 부산진구, 6. 동래구, 7. 남구, 8. 북구, 9. 해운대구, 10. 사하구, 11. 금정구, 12. 강서구, 13. 연제구, 14.수영구, 15. 사상구, 16. 기장구")

        adress = []                 #주소추가되는 리스트

        ad = ['중구', '서구', '동구', '영도구', '부산진구', '동래구', '남구', '북구', '해운대구', '사하구', '금정구', '강서구', '연제구', '수영구', '사상구', '기장구']
        ad_len = len(ad)
        #num input받고
        ad_num = num_input
    ...
    if GPIO.event_detected(Button_next):
        if ad[0] in adress: #중구
            ...
            ad_gsg = ['대저1동', '강동동', '명지동', '죽림동', '식만동', '죽동동', '봉림동', '송정동', '화전동', '녹산동', '생곡동', '구랑동', '지사동','미음동', '범방동', '신호동', '동선동', '성북동', '눌차동', '천성동', '대항동']
            print("주소를 선택해 주세요")
            ad_num = num_input
            numsize = len(ad_gsg)
